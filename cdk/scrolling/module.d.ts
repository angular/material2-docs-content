import { CdkVirtualScrollContextExample } from './cdk-virtual-scroll-context/cdk-virtual-scroll-context-example';
import { CdkVirtualScrollCustomStrategyExample } from './cdk-virtual-scroll-custom-strategy/cdk-virtual-scroll-custom-strategy-example';
import { CdkVirtualScrollDataSourceExample } from './cdk-virtual-scroll-data-source/cdk-virtual-scroll-data-source-example';
import { CdkVirtualScrollDlExample } from './cdk-virtual-scroll-dl/cdk-virtual-scroll-dl-example';
import { CdkVirtualScrollFixedBufferExample } from './cdk-virtual-scroll-fixed-buffer/cdk-virtual-scroll-fixed-buffer-example';
import { CdkVirtualScrollHorizontalExample } from './cdk-virtual-scroll-horizontal/cdk-virtual-scroll-horizontal-example';
import { CdkVirtualScrollOverviewExample } from './cdk-virtual-scroll-overview/cdk-virtual-scroll-overview-example';
import { CdkVirtualScrollTemplateCacheExample } from './cdk-virtual-scroll-template-cache/cdk-virtual-scroll-template-cache-example';
import * as i0 from "@angular/core";
import * as i1 from "./cdk-virtual-scroll-context/cdk-virtual-scroll-context-example";
import * as i2 from "./cdk-virtual-scroll-custom-strategy/cdk-virtual-scroll-custom-strategy-example";
import * as i3 from "./cdk-virtual-scroll-data-source/cdk-virtual-scroll-data-source-example";
import * as i4 from "./cdk-virtual-scroll-dl/cdk-virtual-scroll-dl-example";
import * as i5 from "./cdk-virtual-scroll-fixed-buffer/cdk-virtual-scroll-fixed-buffer-example";
import * as i6 from "./cdk-virtual-scroll-horizontal/cdk-virtual-scroll-horizontal-example";
import * as i7 from "./cdk-virtual-scroll-overview/cdk-virtual-scroll-overview-example";
import * as i8 from "./cdk-virtual-scroll-template-cache/cdk-virtual-scroll-template-cache-example";
import * as i9 from "@angular/cdk/scrolling";
export { CdkVirtualScrollContextExample, CdkVirtualScrollCustomStrategyExample, CdkVirtualScrollDataSourceExample, CdkVirtualScrollDlExample, CdkVirtualScrollFixedBufferExample, CdkVirtualScrollHorizontalExample, CdkVirtualScrollOverviewExample, CdkVirtualScrollTemplateCacheExample, };
export declare class CdkScrollingExamplesModule {
    static ngModuleDef: i0.ɵɵNgModuleDefWithMeta<CdkScrollingExamplesModule, [typeof i1.CdkVirtualScrollContextExample, typeof i2.CdkVirtualScrollCustomStrategyExample, typeof i3.CdkVirtualScrollDataSourceExample, typeof i4.CdkVirtualScrollDlExample, typeof i5.CdkVirtualScrollFixedBufferExample, typeof i6.CdkVirtualScrollHorizontalExample, typeof i7.CdkVirtualScrollOverviewExample, typeof i8.CdkVirtualScrollTemplateCacheExample], [typeof i9.ScrollingModule], [typeof i1.CdkVirtualScrollContextExample, typeof i2.CdkVirtualScrollCustomStrategyExample, typeof i3.CdkVirtualScrollDataSourceExample, typeof i4.CdkVirtualScrollDlExample, typeof i5.CdkVirtualScrollFixedBufferExample, typeof i6.CdkVirtualScrollHorizontalExample, typeof i7.CdkVirtualScrollOverviewExample, typeof i8.CdkVirtualScrollTemplateCacheExample]>;
    static ngInjectorDef: i0.ɵɵInjectorDef<CdkScrollingExamplesModule>;
}
