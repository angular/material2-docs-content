...
class CdkTreeFlatChildrenAccessorExample {
  ...
  static ɵcmp = i0.ɵɵngDeclareComponent({
    ...
    dependencies: [{
      ...
      kind: "directive",
      type: i1.CdkTreeNodeDef,
      selector: "[cdkTreeNodeDef]",
      inputs: ["cdkTreeNodeDefWhen"]
    }, {
      kind: "directive",
      type: i1.CdkTreeNodePadding,
      selector: "[cdkTreeNodePadding]",
      inputs: ["cdkTreeNodePadding", "cdkTreeNodePaddingIndent"]
    }, {
      kind: "directive",
      type: i1.CdkTreeNodeToggle,
      selector: "[cdkTreeNodeToggle]",
      inputs: ["cdkTreeNodeToggleRecursive"]
    }, {
      kind: "component",
      type: i1.CdkTree,
      selector: "cdk-tree",
      inputs: ["dataSource", "treeControl", "levelAccessor", "childrenAccessor", "trackBy", "expansionKey"],
      exportAs: ["cdkTree"]
    }, {
      kind: "directive",
      type: i1.CdkTreeNode,
      selector: "cdk-tree-node",
      inputs: ["role", "isExpandable", "isExpanded", "isDisabled", "cdkTreeNodeTypeaheadLabel"],
...
class TransformedData {
  ...
  isExpandable() {
    return (this.raw.childrenLoading === 'INIT' || this.raw.childrenLoading === 'LOADED') && !!this.raw.childrenIds?.length;
  }
  isLeaf() {
...
class ComplexDataStore {
  ...
  _setChildrenLoading(parentId) {
    ...
    this._state.next({
      ...currentState,
      allData: new Map([...currentState.allData, ...(parentData ? [[parentId, {
        ...parentData,
        childrenLoading: 'LOADING'
      }]] : [])]),
      dataLoading: new Map([...currentState.dataLoading, ...(parentData?.childrenIds?.map(childId => [childId, 'LOADING']) ?? [])])
    });
  }
  ...
  _addData({
    ...
    return {
      allData: new Map([...allData, ...data.map(datum => {
        return [datum.id, {
          id: datum.id,
          name: datum.name,
          parentId,
        ...
        ...parentData,
        childrenLoading: 'LOADED'
      }]] : [])]),
      dataLoading: new Map([...dataLoading, ...data.map(datum => [datum.id, 'LOADED']), ...allChildren.map(childId => [childId, 'INIT'])])
    };
  }
  _getDataByIds(ids, data) {
    return ids.map(id => data.get(id)).filter(item => !!item).map(datum => new TransformedData(datum));
  }
  select(...sourcesAndTransform) {
    ...
    return combineLatest([...sources, this._state]).pipe(map(args => transformFn(...args)), shareReplay({
      refCount: true,
      bufferSize: 1
    }));
  }
...
class VimTreeKeyManager {
  ...
  focusItem(itemOrIndex, options = {}) {
    ...
    let index = typeof itemOrIndex === 'number' ? itemOrIndex : this._items.findIndex(item => item === itemOrIndex);
...
//# sourceMappingURL=cdk-tree.mjs.map
